...
const ProfileFavorite = ({
  itinerary,
  showHeart,
  onRemove = () => {}, // <-- Default to no-op
  disabled,
}) => {
  const { image, trip_title, destination, id } = itinerary;
  const navigate = useNavigate();

  // Prevent card navigation when clicking the heart
  const handleCardClick = (e) => {
    // If the heart button was clicked, do not navigate
    if (e.target.closest("button")) return;
    navigate(`/itinerary-details/${id}`);
  };
      ...
      <div className="px-4 py-3 flex items-center justify-between">
        ...
        {showHeart && (
          <button
            ...
            className="ml-2 text-[#F43F5E] hover:scale-110 transition"
